...
class SystemMaintenance:
    ...
    def run_maintenance(logger_func):
        ...
        with open(logfile, "w", encoding="utf-8") as log:
            ...
            logger_func("🗑️ Limpiando archivos temporales...")
            SystemMaintenance._clean_temp_files(logger_func, log)

            logger_func("🖥️ Recolectando información del sistema...")
            ...
            logger_func("⏱️ Ejecutando test de latencia en core 0 (10 pruebas)...")
            ...
            logger_func("⚙️ Listando programas en Startup...")
            SystemMaintenance._list_startup_programs(logger_func, log)

        logger_func("🎯 MANTENIMIENTO COMPLETADO EXITOSAMENTE")
        logger_func(f"📂 Log guardado en {log_filename}")
    ...
    def _clean_temp_files(logger_func, log_file):
        ...
        for path in temp_paths:
            if not os.path.exists(path):
                logger_func(f"⚠️ Directorio no encontrado: {path}")
            ...
            try:
                    ...
                    for file in files:
                        ...
                        try:
                            ...
                            if total_deleted % 500 == 0:
                                logger_func(f"📊 Eliminados {total_deleted} archivos...")
                ...
                logger_func(f"⚠️ Error al acceder al directorio {root}: {str(e)}")
                log_file.write(f"Error al acceder al directorio {root}: {str(e)}\n")

        logger_func(f"📊 RESUMEN: {total_deleted} archivos/directorios eliminados, {total_skipped} omitidos")
    ...
    def _run_disk_benchmark(logger_func, log_file):
        ...
        try:
            ...
            logger_func(f"❌ Error en benchmark: {e}")
    ...
    def _run_latency_test(logger_func, log_file):
        ...
        try:
            for i in range(10):
                ...
                logger_func(f"⏱️ Chequeo {i+1}: {latency} ns")
                log_file.write(f"Chequeo {i+1}: {latency} ns\n")
                log_file.flush()
                time.sleep(0.5)
        except Exception as e:
            logger_func(f"❌ Error en latencia: {e}")
        finally:
